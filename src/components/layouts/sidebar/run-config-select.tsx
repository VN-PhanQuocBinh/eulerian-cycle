  ...
  return (
    <section className={cn("", className)}>
      ...
      <Select.Root defaultValue={currentValue} value={currentValue as T} onValueChange={onSelect}>
        <Select.Trigger
          className="w-full flex items-center data-placeholder:text-(--gl-text-muted) data-placeholder:italic justify-between px-3 py-2 bg-(--gl-bg-base) border border-(--gl-border) rounded-lg text-sm text-(--gl-text-main) hover:border-(--gl-border) focus:outline-none focus:ring-2 focus:ring-(--gl-blue-dark) transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          disabled={isAnimating || options.length === 0}
        >
          <Select.Value placeholder={placeholder} defaultValue={currentValue} className="" />

          <Select.Icon>
            <ChevronDown size={16} className="text-(--gl-text-muted)" />
          </Select.Icon>
        </Select.Trigger>
            ...
            {options.map((option) => (
              <SelectItem key={option.value} value={option.value}>
                <div>
                  ...
                  {/* <div className="text-xs text-(--gl-text-muted)">
