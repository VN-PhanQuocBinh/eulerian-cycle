...
export function BottomPanel() {
  ...
  useRegisterHotkey({
    type: "click",
    combo: HOTKEYS_CONFIG.CLICK.TOGGLE_RIGHT_SIDEBAR,
    handler: () => toggleRightSidebar(!isRightSidebarOpen),
  });
  ...
  return (
    <Tabs
      ref={tabsRootRef}
      defaultValue="steps"
      className="h-full tabs-bar px-2 relative bg-(--gl-bg-surface) gap-0 rounded-md"
      ...
      <div className="flex items-center mb-2 py-1">
        <TabsList variant="line" className="">
          {tabs.map((tab) => (
            <TabsTrigger
              key={tab.value}
              value={tab.value}
              className="text-[12px] font-semibold rounded-sm px-3"
            >
              {tab.label}
            </TabsTrigger>
          ))}
        ...
        <div className="flex-1 flex justify-end items-center gap-1 text-right text-sm ">
          {bottomPanelTab === "steps" && (
            <>
              <Button variant="ghost" size="sm" onClick={toggleSmartScroll} className="text-[12px]">
                {enableSmartScroll ? (
          ...
          <Tooltip content={`Close Panel`} side="top">
            <Button
              variant="ghost"
              size="sm"
