...
export default function TopMenuBar() {
  ...
  return (
    <header className="max-w-full window-drag min-h-6 bg-(--gl-bg-base) text-(--gl-text-main)">
      <div className="relative flex h-full items-center justify-between pl-2 pr-0.5">
        <div className="window-no-drag shrink-0 flex items-center h-full">
          ...
          >
            <PopoverTrigger asChild>
              <button
                onClick={openFile}
                className="rounded! px-2.5 py-0.5 text-[12px] font-semibold text-(--gl-text-main) hover:bg-(--gl-bg-subtle) hover:text-(--gl-text-main)"
              >
                File
              </button>
            </PopoverTrigger>
            <PopoverContent
              ...
              align="start"
              sideOffset={6}
              className="window-no-drag w-60 border-(--gl-border) bg-(--gl-bg-base) p-0.5"
            >
              <button
          ...
          >
            ...
            <PopoverContent
              ...
              align="start"
              sideOffset={6}
              className="window-no-drag w-56 border-(--gl-border) bg-(--gl-bg-base) p-1"
            >
              <button
        ...
        >
          {hotkeyLog.length > 0 &&
            hotkeyLog.map((log, index) => (
              <div className="rounded-sm bg-(--gl-bg-base) px-4" key={index}>
                {log}
              </div>
            ))}
        </div>
        ...
        <div className="window-no-drag shrink-0 flex items-stretch">
          <button
            ...
            className="grid h-7 w-13 place-items-center rounded-none! text-(--gl-text-main) hover:bg-(--gl-bg-subtle) hover:text-(--gl-text-main)"
          >
            <Minus size={16} />
          </button>
          <button
          ...
          <button
            ...
            className="grid h-7 w-13 place-items-center rounded-none! text-(--gl-text-main) hover:bg-(--gl-red-dark) hover:text-(--gl-text-main)"
          >
            <X size={16} />
          </button>
        </div>
